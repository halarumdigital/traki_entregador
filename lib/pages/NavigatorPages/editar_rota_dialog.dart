...
class EditarRotaDialog extends StatefulWidget {
  final MinhaRota rota;

  const EditarRotaDialog({
    super.key,
    required this.rota,
  });

  @override
  State<EditarRotaDialog> createState() => _EditarRotaDialogState();
}

class _EditarRotaDialogState extends State<EditarRotaDialog> {
  final _formKey = GlobalKey<FormState>();
  late TextEditingController _capacidadePacotesController;
  late TextEditingController _capacidadePesoController;
  late TimeOfDay _horarioSaida;
  late Set<int> _diasSelecionados;
  late bool _ativo;
  ...
  void initState() {
    super.initState();
    _capacidadePacotesController = TextEditingController(
      text: widget.rota.capacidadePacotes.toString(),
    );
    _capacidadePesoController = TextEditingController(
      text: widget.rota.capacidadePesoKg,
    );

    // Parse horário
    final horarioParts = widget.rota.horarioSaidaPadrao.split(':');
    _horarioSaida = TimeOfDay(
      hour: int.parse(horarioParts[0]),
      minute: int.parse(horarioParts[1]),
    );

    // Inicializar dias selecionados (padrão seg-sex se vazio)
    debugPrint('📅 DEBUG Editar - diasSemana da rota: ${widget.rota.diasSemana}');
    _diasSelecionados = widget.rota.diasSemana.isEmpty
        ? {1, 2, 3, 4, 5}
        : widget.rota.diasSemana.toSet();
    debugPrint('📅 DEBUG Editar - dias selecionados iniciais: $_diasSelecionados');

    _ativo = widget.rota.ativo;
  }
  ...
  Future<void> _salvar() async {
    ...
    try {
      ...
      final resultado = await RotasService.atualizarRota(
      ...
      if (resultado != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Erro ao atualizar rota. Tente novamente.'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Erro ao atualizar rota. Verifique os valores.'),
          backgroundColor: Colors.red,
        ),
      );
      setState(() {
