...
//get language json and data saved in local (bearer token , choosen language) and find users current status
  getLanguageDone() async {
    ...
    try {
      ...
      //       .get();
      // }
      _version = null; // Firebase Database removed
      _error = false;
      // Firebase Database removed - version check disabled
      ...
      if (updateAvailable == false) {
        ...
        // Garantir que o idioma padrão português esteja salvo
        if (!pref.containsKey('choosenLanguage')) {
          await pref.setString('choosenLanguage', 'pt');
          await pref.setString('languageDirection', 'ltr');
          choosenLanguage = 'pt';
        ...
        if (internet == true) {
          ...
          if (val == '3') {
            ...
            // Caso não tenha idioma salvo (val == '1'), vai direto para Login
            ischeckownerordriver = 'driver';
            Future.delayed(const Duration(seconds: 2), () {
              Navigator.pushReplacement(context,
                  MaterialPageRoute(builder: (context) => const Login()));
