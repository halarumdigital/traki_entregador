...
import 'package:flutter_driver/pages/login/register_step1_personal.dart';
import 'package:flutter_driver/pages/onTripPage/rides.dart';
import 'package:flutter_driver/pages/home_simple.dart';
...
//pick image from camera
  ...
  navigate(verify) async {
    debugPrint('🔍 navigate() chamado com verify: $verify');
    debugPrint('📦 userDetails: $userDetails');

    if (verify == true) {
      debugPrint('✅ verify == true');
      debugPrint('📄 uploaded_document: ${userDetails['uploaded_document']}');
      debugPrint('✔️ approve: ${userDetails['approve']}');
      debugPrint('👤 role: ${userDetails['role']}');
      debugPrint('🎯 enable_bidding: ${userDetails['enable_bidding']}');

      // 💾 Salvar sessão local
      try {
        // Obter o token - deve estar disponível agora após driverLogin
        String? token;
        ...
        if (token == null || token.isEmpty) {
          debugPrint('⚠️ Token não disponível ao salvar sessão');
          token = ''; // Salvar com token vazio por enquanto
        }
        ...
        await LocalStorageService.saveDriverSession(
          ...
          driverData: {
            ...
            'uploadedDocuments': userDetails['uploaded_document'] ?? false,
            'role': userDetails['role'],
            'enable_bidding': userDetails['enable_bidding'] ?? false,
            'available': userDetails['available'] ?? false,
          },
        );
        debugPrint('💾 Sessão salva localmente com token');
      } catch (e) {
        debugPrint('❌ Erro ao salvar sessão: $e');
      }

      if (userDetails['uploaded_document'] == false) {
        debugPrint('➡️ Navegando para ApprovalStatusScreen (documento não enviado)');
        ...
        debugPrint('➡️ Navegando para ApprovalStatusScreen (não aprovado)');
        Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
              builder: (context) => ApprovalStatusScreen(
        ...
        debugPrint('✅ Documento enviado e aprovado');
        if (userDetails['role'] != 'owner' &&
            userDetails['enable_bidding'] == true) {
          debugPrint('➡️ Navegando para RidePage (bidding ativo)');
          Navigator.pushAndRemoveUntil(
              context,
          ...
          debugPrint('➡️ Navegando para HomeSimple (bidding desativado)');
          Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => const HomeSimple()),
              (route) => false);
        } else {
          debugPrint('➡️ Navegando para HomeSimple (owner ou padrão)');
          Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(builder: (context) => const HomeSimple()),
              (route) => false);
      ...
      if (verify.toString().contains('aguardando aprovação') ||
          verify.toString().contains('aguardando aprobação')) {
        debugPrint('🔄 Erro 403 detectado - verificando dados locais...');
        ...
        try {
          ...
          if (driverData != null && driverData['id'] != null) {
            ...
            debugPrint('👤 Driver ID encontrado: $driverId');
            debugPrint('📄 Documentos enviados: $uploadedDocuments');

            // Redirecionar para tela de status/documentos
            Navigator.pushAndRemoveUntil(
              context,
              MaterialPageRoute(
                builder: (context) => ApprovalStatusScreen(
          ...
          debugPrint('❌ Erro ao verificar dados locais: $e');
        }
      }

      // Se não foi possível redirecionar, mostrar erro
      setState(() {
                                              ...
                                              SizedBox(
                                                ...
                                                child: Row(
                                                  ...
                                                  children: [
                                                    ...
                                                    InkWell(
                                                        onTap: () {
                                                          // Navegar para o fluxo de cadastro em 3 etapas
                                                          Navigator.push(
                                                            context,
                                                            MaterialPageRoute(
                                                              builder: (context) =>
